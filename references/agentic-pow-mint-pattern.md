...
## Question format variants (Alien Node 2026-08-17 — solver di `scripts/aliennode-mint.js`)
- `"What is 55 - 38?"` (add/sub/mul/div/mod: `"What is 83 mod 7?"`)
- `"What is half of 106?"` / `"What is double X?"` / `"What is X squared?"`
- Three-term: `"What is 18 + 5 + 10?"` → eval aman: replace ×÷− lalu regex `^[\d\s+\-*/%().]+$` sebelum `Function()` (no letters = safe)
- **Base-convert pake kata "Convert", BUKAN "What is":** `"Convert 128 to hexadecimal (no 0x prefix, lowercase)."` → `Number(x).toString(16)`; `"Convert 53 to binary (no prefix)."` → `toString(2)`; tambahin octal `toString(8)` buat jaga-jaga. Kalau solver belum kenal format → error "Puzzle format tak dikenal" → cek log, tambah pattern, retry wallet yang gagal aja.
